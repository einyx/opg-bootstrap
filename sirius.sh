...
if [[ -z $SALT_MASTER ]]; then
    ...
    if [[ -z $SALT_MASTER_IP ]] || [[ $ROLE == 'master' ]]; then
        SALT_MASTER='yes'
    fi
fi
...
# Get the list of Ephemeral and EBS devices from Amazon ...
EPHEMERALSANDEBS=($(
    curl -s ${EC2_METADATA_URL}/block-device-mapping/ | \
        awk '/(ephemeral|ebs)[[:digit:]]+/ { print }'
))
...
if [[ $EC2_AUTO_SCALING_GROUP == 'yes' ]]; then
    ...
    HOSTNAME="${ROLE}-$(echo $INSTANCE_ID | sed -e 's/^i\-//')"
...
EOF
    ...
    update-rc.d -f salt-master disable || true
...
# Start Minion after making sure that Master is already
# running (at least it is accepting connections).
